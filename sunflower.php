...
$ussserid_num=mysql_num_rows(mysql_query("SELECT * FROM `mlm_register` WHERE `user_sponserid`='$ussserid' and user_profileid!='$ussserid' order by user_id asc"));
	if($ussserid_num > 0){
		$i = 1;
		$j = 1;
		$k = 0;
...
$ussserid_num=mysql_num_rows(mysql_query("SELECT * FROM `mlm_register` WHERE `user_sponserid`='$sponsor' and user_profileid!='$sponsor' order by user_id asc"));
	if($ussserid_num > 0){
		while($fetch_ussserid = mysql_fetch_array($select_ussserid)){
		//echo $z;
			$level2[$i][$j] = $fetch_ussserid['user_fname']." [". $fetch_ussserid['user_profileid'] ."]";
			$level3[$z+$j][$k] = $fetch_ussserid['user_fname']." [". $fetch_ussserid['user_profileid'] ."]";
...
$ussserid_num=mysql_num_rows(mysql_query("SELECT * FROM `mlm_register` WHERE `user_sponserid`='$sponsor' and user_profileid!='$sponsor' order by user_id desc"));
	if($ussserid_num > 0){
		while($fetch_ussserid = mysql_fetch_array($select_ussserid)){
			$level12[$i][$j] = $fetch_ussserid['user_fname']." [". $fetch_ussserid['user_profileid'] ."]";
